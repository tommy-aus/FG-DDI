...
def test(s1_data_loader, s2_data_loader, model):
    ...
    with torch.no_grad():
        for batch in s1_data_loader:
            model.eval()
            p_score, n_score, probas_pred, ground_truth, rel_types = do_compute(batch, device, model=model)
            s1_probas_pred.append(probas_pred)
            s1_ground_truth.append(ground_truth)
            s1_rel_types.append(rel_types)
        ...
        # Get overall metrics
        s1_metrics = do_compute_metrics(s1_probas_pred, s1_ground_truth)
        s1_acc, s1_auc_roc, s1_f1, s1_precision, s1_recall, s1_int_ap, s1_ap = s1_metrics
        
        # Get per-relation metrics
        _, _, _, _, _, _, _, s1_rel_metrics = do_compute_metrics(
            s1_probas_pred, s1_ground_truth, s1_rel_types, per_rel=True)
        ...
        # Get overall metrics
        s2_metrics = do_compute_metrics(s2_probas_pred, s2_ground_truth)
        s2_acc, s2_auc_roc, s2_f1, s2_precision, s2_recall, s2_int_ap, s2_ap = s2_metrics
        
        # Get per-relation metrics
        _, _, _, _, _, _, _, s2_rel_metrics = do_compute_metrics(
            s2_probas_pred, s2_ground_truth, s2_rel_types, per_rel=True)
    ...
    for rel, metrics_dict in s1_rel_metrics.items():
    ...
    for rel, metrics_dict in s2_rel_metrics.items():
        if isinstance(metrics_dict, dict) and 'error' not in metrics_dict:
            adr_data.append({
                'ADR_Type': rel,
                'Accuracy': metrics_dict['acc'],
    ...
    # Save to CSV
    final_adr_df.to_csv('per_adr_metrics.csv', index=False)
    print("Per-ADR metrics saved to 'per_adr_metrics.csv'")
    
    return (s1_rel_metrics, s2_rel_metrics), final_adr_df
